...
#正向模式：基于物理方程（RK4 方法）沿时间正方向推进，适合常规模拟。
#反向模式：结合神经网络沿时间反方向迭代，可能用于误差修正、粒子溯源或数据驱动的模拟校正。
...
#test 结束
    ...
    if from_frame <= 0:
        ...
        #检验是否初始化
        smoke_np = smoke.to_numpy()
        print("stripe_func 执行后烟雾值范围：", smoke_np.min(), smoke_np.max())
...
# 粒子位置初始化：init_particles_pos_uniform在网格中均匀分布粒子，参数包括：
# particles_per_cell：每个网格单元的粒子数
# dist_between_neighbor：粒子间最小距离
# 粒子属性初始化：
# init_particles_imp：设置粒子的冲量（impulse）
# init_particles_imp_grad_m：设置粒子梯度相关的冲量属性
# reset_T_to_identity：重置变换矩阵为单位矩阵（初始无变形）
# 数据备份：backup_particles_pos保存粒子初始位置，用于后续恢复或对比
...
# get_central_vector：从网格速度分量计算中心速度
# curl：计算涡度场（流体旋转强度）
# 可视化输出：
# write_field：将涡度场保存为图像，可选择叠加粒子位置
# write_image：保存烟雾密度场图像
    ...
    sub_t = 0.  # 自上次重初始化后的时间
    frame_idx = from_frame  # 当前帧索引
    last_output_substep = 0  # 上次输出帧的子步数
    num_reinits = 0  # 已执行的重初始化次数
    i = -1   # 主循环计数器（从-1开始，第一次迭代前+1变为0）
    ik = 0   # 用于记录最近一次重初始化的步骤索引

    frame_times = np.zeros(total_steps) 
#初始化一个长度为 total_steps 的 NumPy 数组，用于记录每一步模拟的执行时间
    ...
    while True:
        ...
        #平流后烟雾是否正常
        smoke_np = smoke.to_numpy()
        print("平流后烟雾值范围：", smoke_np.min(), smoke_np.max())  # 若输出 0.0 0.0，说明平流逻辑错误
