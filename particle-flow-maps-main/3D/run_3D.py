...
particles_per_cell_axis = 2 #每个网格单元沿着轴的粒子数
dist_between_neighbor = dx / particles_per_cell_axis  #粒子间的距离

one_sixth = 1. / 6  #RK4积分的系数 （1/6）？？

# solver 边界条件与泊松求解器初始化（3D多重网格CG solver）？？？
...
# undeformed coordinates (cell center and faces) 网格坐标存储（为变形的初始坐标：单元格中心、xyz方向面中心坐标）
...
particles_smoke = ti.Vector.field(4, float, shape=particle_num)#存储每个粒子携带的烟雾属性（ti.Vector.field(4, float)：4D 向量，常见用途是[r, g, b, alpha]（RGBA 颜色，包含透明度）
particles_init_grad_smoke = ti.Matrix.field(3, 4, float, shape=particle_num)#存储每个粒子的烟雾属性初始梯度矩阵
...
#     # 日志目录
#     logsdir = os.path.join(script_dir, "logs", "exp")
#     os.makedirs(logsdir, exist_ok=True)
...
if __name__ == '__main__':
    ...
    # Auto-generate 3d_output (volume rendering like ParaView)
    try:
        logsdir = os.path.join('logs', exp_name)
        if os.path.isdir(os.path.join(logsdir, 'vtks')):
            print("\n[Main] Generating 3d_output volume rendering...")
            sys.path.insert(0, os.path.dirname(__file__))
            from vtk_visualize import visualize
            visualize(os.path.abspath(logsdir), fields=('vorticity',), azim=30, elev=25)
    except Exception as e:
        print(f"[Main] 3d_output generation skipped: {e}")
        print("[Main] Run manually: python 3D/vtk_visualize.py " + os.path.join('logs', exp_name))
